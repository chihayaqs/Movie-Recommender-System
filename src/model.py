...
def _cosine_similarity_by_columns(matrix_values: np.ndarray) -> np.ndarray:
	"""Compute cosine similarity between columns of a 2D numpy array.

	Expects shape (n_users, n_items). Missing values should be filled with 0.
	Returns (n_items, n_items) similarity matrix.
	"""
...
def compute_item_similarity(matrix: pd.DataFrame) -> Tuple[np.ndarray, List[int]]:
	"""Compute item-item cosine similarity.

	Returns (sim_matrix, item_ids) where item_ids matches column order in sim_matrix.
	"""
	item_ids = list(matrix.columns)
	values = matrix.fillna(0).values  # shape (n_users, n_items)
...
def predict_ratings_item_based(
	matrix: pd.DataFrame, sim: np.ndarray, item_ids: List[int], user_id: int, top_k: int = 20
) -> pd.Series:
	...
	# map item id to column index
	id_to_idx = {it: idx for idx, it in enumerate(item_ids)}

	user_vector = user_ratings.fillna(0).values  # length n_items
	rated_indices = np.array([id_to_idx[it] for it in rated_items], dtype=int)
	...
	for j, item in enumerate(item_ids):
		...
		sims = sim[j]
		selected_indices = _top_k_similar_indices(sims, rated_indices, top_k)
		if selected_indices.size == 0:
			preds[item] = np.nan
			continue
		numer = np.dot(sims[selected_indices], user_vector[selected_indices])
		denom = np.sum(np.abs(sims[selected_indices]))
		if denom > 0:
